...
Clone the repository, then in its base folder type ```make install```. This compiles the FORTRAN code and runs ```pip install``` in [editable mode](https://pip.pypa.io/en/latest/reference/pip_install/#editable-installs), which means you have the freedom to edit the python code. After that, feel free to run unit tests with ```make test``` to check that all went well.

`pyne2001` works with both python 2.7 and python 3+, and has no external dependencies.


### Usage

`pyne2001` provides 5 functions, see their docstrings for the full details:
